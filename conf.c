...
static int ctable_count = sizeof(ctable) / sizeof(struct conftab);
static int ctable_sorted = 0; /* Should just initialize it sorted. */

static int ctable_cmp(s1, s2)
    const VOID *s1;
    const VOID *s2;
{
  const struct conftab *c1 = s1;
  const struct conftab *c2 = s2;

  return(strcasecmp(c1->key, c2->key));
}
    ...
    char *fname;
{
  ...
  if (!ctable_sorted)
    qsort(ctable, ctable_count, sizeof(struct conftab), ctable_cmp);
    ...
    for(index = 0; index < 64; index++) {
      if(val[index] != (char *)NULL) {
        if(strchr(val[index], ' ') == (char *)NULL) {
          strlcat(buffer, val[index], (blen - strlen(buffer) - 1));
	  strlcat(buffer, ", ", (blen - strlen(buffer) - 1));
        } else {
    ...
    char *argone, *argtwo;
{
  ...
  if(switches & CONFIG_SET) {
    ...
    f.key = argone;
    cptr = bsearch(&f, ctable, ctable_count, sizeof(struct conftab), ctable_cmp);
    if (cptr != NULL) {
