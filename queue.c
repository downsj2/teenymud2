    ...
    char *eargv[];
{
  ...
  if(mudconf.enable_money) {
    ...
    if(check >= mudconf.queue_commands) {
      if(!can_afford(owner, cause, mudconf.queue_cost))
	return;
    ...
    char *argone, *argtwo;
{
  ...
  if((argone == (char *)NULL) || (argone[0] == '\0')) {
    notify_player(player, cause, player, "Notify what?", 0);
    return;
  }
  if((argtwo == (char *)NULL) || (argtwo[0] == '\0')) {
    ...
    if(ptr == argtwo) {
      notify_player(player, cause, player, "I don't understand that count.", 0);
      return;
    }
    if(count == 0) {
  ...
  notify_player(player, cause, player, "Notified.", 0);
}
    ...
    char *arg;
{
  ...
  if(switches & HALT_ALL) {	/* expunge the queue(s). */
    ...
    victim = resolve_object(player, cause, arg, RSLV_NOISY|RSLV_EXITS);
    if(victim == -1)
      return;
  ...
  notify_player(player, cause, player, "Halted.", 0);
}
    ...
    char *argone, *argtwo;
{
  ...
  if((argone == (char *)NULL) || (argone[0] == '\0')
    ...
    notify_player(player, cause, player,
		  "You must specify an object and a command.", 0);
    return;
  }
  ...
  if(*ptr == '/') {
    *ptr++ = '\0';
    ...
    if(nptr == ptr) {
      notify_player(player, cause, player, 
		    "You must specify delay by number.", 0);
      return;
    }
  } else
    delay = -1;
  ...
  notify_player(player, cause, player, "Queued.", 0);
}
    ...
    char *argone, *argtwo;
{
  ...
  if((argone == (char *)NULL) || (argone[0] == '\0')
     || (argtwo == (char *)NULL) || (argtwo[0] == '\0')) {
    notify_player(player, cause, player, 
		  "You must specify a delay and a command.", 0);
    return;
  }
  ...
  if(*ptr == '/') {	/* optional object. */
    ptr++;
    obj = resolve_object(player, cause, ptr, RSLV_NOISY|RSLV_EXITS);
    if(obj == -1)
      return;
    ...
    queue_add(obj, obj, delay, -1, argtwo, 0, (char **)NULL);
  } else
    queue_add(player, cause, delay, -1, argtwo, 0, (char **)NULL);
  notify_player(player, cause, player, "Queued.", 0);
}
    ...
    char *arg;
{
  ...
  if(switches & PS_ALL) {
    ...
    if((arg == (char *)NULL) || (arg[0] == '\0')) {
      ...
      searchfor = resolve_object(player, cause, arg, RSLV_NOISY|RSLV_EXITS);
      if(searchfor == -1)
        return;
    }
  }
    ...
    char *argone, *argtwo;
{
  ...
  if (!argone || !*argone || !argtwo || !*argtwo) {
    notify_player(player, cause, player, "Force who to do what?", 0);
    return;
  }
  ...
  if (isGOD(obj) && !isGOD(player)) {
    notify_player(player, cause, player,
		  "You can't force God to do anything!", 0);
    return;
  }
    ...
    char *argv[];
{
  ...
  if(argc < 2) {
    notify_player(player, cause, player, "Trigger what?", 0);
    return;
  }

  obj = resolve_object(player, cause, argv[0], RSLV_NOISY|RSLV_EXITS);
  if(obj == -1)
    return;
  if(!controls(player, cause, obj)) {
    notify_player(player, cause, player, "Permission denied.", 0);
  ...
  notify_player(player, cause, player, "Triggered.", 0);
}
    ...
    char *argone, *argtwo;
{
  ...
  if((argone == (char *)NULL) || (argone[0] == '\0')
     || (argtwo == (char *)NULL) || (argtwo[0] == '\0')) {
    notify_player(player, cause, player,
		  "You can't foreach without a list.", 0);
    return;
  }
  ...
  if(*ptr == '/') {
    *ptr++ = '\0';
    ...
    notify_player(player, cause, player,
    		  "You must specify a variable name.", 0);
    return;
  }
    ...
    char *argv[];
{
  ...
  if(argc < 3) {
    notify_player(player, cause, player, "How can you case on that?", 0);
    return;
  }
  ...
  while(1) {
    if((idx == (argc-1)) && !done) {	/* default */
      queue_add(player, cause, 1, -1, argv[idx], 0, (char **)NULL);
      break;
    } else if(idx == (argc-1))		/* simply done */
      break;
