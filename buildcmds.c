    ...
    int obj, new;
{
  ...
  for(idx = 0; idx < Ltable_count; idx++) {
    ...
    char *argone;
{
  ...
  if (argone[0] == '\0') {
    if(!(switches & CMD_QUIET))
      notify_player(player, cause, player, "You must specify an object.",
		    NOT_QUIET);
    return;
  }
    ...
    char *argone, *argtwo;
{
  ...
  argone = parse_slash(argone, &satr);
  if (satr != (char *)NULL) {
  ...
  argtwo = parse_slash(argtwo, &datr);
  if (datr != (char *)NULL) {
