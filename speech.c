    ...
    char *argone;
{
  ...
  notify_flags = (NOT_NOPUPPET | NOT_NOSPOOF);
  if (switches & EMIT_HTML)
    notify_flags |= NOT_RAW;

  if(switches & EMIT_OTHERS) {
    notify_oall(player, cause, player, argone, notify_flags);
    if(!(switches & CMD_QUIET))
      notify_player(player, cause, player, "Ok.", NOT_QUIET);
  } else {
    notify_all(player, cause, player, argone, notify_flags);
  }
}
    ...
    char *argone, *argtwo;
{
  ...
  notify_flags = NOT_NOSPOOF;
  if(switches & PEMIT_HTML)
    notify_flags |= NOT_RAW;

  notify_player(victim, cause, player, argtwo, notify_flags);
    ...
    char *argone, *argtwo;
{
  ...
  pagee = resolve_player(player, cause, argone,
  		 (!(switches & CMD_QUIET) ? RSLV_NOISY|RSLV_ALLOK|RSLV_SPLATOK
			 		  : RSLV_ALLOK|RSLV_SPLATOK));
  if(pagee == -1)
    return;
  if (!isALIVE(pagee)) {
    if(!(switches & CMD_QUIET))
