...
static void reward_move _ANSI_ARGS_((int, int, int));
    ...
    int player, cause, switches;
{
  ...
  if (!isDARK(here) && !isDARK(player)) {
    ...
    notify_oall(player, cause, player, buf, 0);
  }
  notify_player(player, cause, player, "There's no place like home...", 0);
  notify_player(player, cause, player, "There's no place like home...", 0);
  notify_player(player, cause, player, "There's no place like home...", 0);
  notify_player(player, cause, player,
  		"You wake up back home, without your possessions.", 0);
    ...
    char *argone;
{
  ...
  if (!argone || !*argone) {
    notify_player(player, cause, player, "You can't go that way.", 0);
    return;
  }
    ...
    char *argone, *argtwo;
{
  ...
  if ((argone == (char *)NULL) || (argone[0] == '\0')
    ...
    notify_player(player, cause, player, "Hand what to who?", 0);
    return;
  }
  ...
  if(obj == -1) {
    notify_player(player, cause, player, "You don't have that.", 0);
    return;
  }
  if(obj == -2) {
    notify_player(player, cause, player, "I can't tell which one you mean.", 0);
    return;
  }
  if(obj == player) {
    notify_player(player, cause, player, "You wouldn't fit!", 0);
    return;
  }
  ...
  if(destination == obj) {
    notify_player(player, cause, player, "It wouldn't fit!", 0);
    return;
  }
  ...
  notify_player(player, cause, player, buffer1, 0);
    ...
    char *argone;
{
  ...
  if ((argone == (char *)NULL) || (argone[0] == '\0')) {
    notify_player(player, cause, player, "Enter what?", 0);
    return;
  }
  if ((get_int_elt(player, LOC, &here) == -1) || !exists_object(here)) {
    notify_player(player, cause, player, noloc, 0);
    ...
    int player, cause, switches;
{
  ...
  if (isROOM(here) || isEXIT(here)) {
    notify_player(player, cause, player, 
		  "You're not inside of anything to leave!", 0);
    return;
  }
    ...
    char *argone, *argtwo;
{
  ...
  if ((argone == (char *)NULL) || (argone[0] == '\0')
      || (argtwo == (char *)NULL) || (argtwo[0] == '\0')) {
    notify_player(player, cause, player, "Attach what to what?", 0);
    return;
  }
  action = resolve_exit(player, cause, argone, RSLV_NOISY);
  if(action == -1)
    return;
  dest = resolve_object(player, cause, argtwo, RSLV_NOISY);
  if(dest == -1)
    return;
  ...
  if (!controls(player, cause, dest) || !has_exits(dest)) {
    notify_player(player, cause, player, 
		  "You can't attach anything to that!", 0);
    return;
  }
  ...
  notify_player(player, cause, player, "Action reattached.", 0);
}
    ...
    char *argone;
{
  ...
  if (!argone || !*argone) {
    notify_player(player, cause, player, "Take what?", 0);
    return;
  }
  if ((get_int_elt(player, LOC, &here) == -1) || !exists_object(here)) {
    notify_player(player, cause, player, noloc, 0);
  ...
  case TYP_PLAYER:
    ...
    break;
  default:
    notify_player(player, cause, player, "You can't take that!", 0);
    return;
  }
  notify_player(player, cause, player, "Taken.", 0);
}
    ...
    char *argone;
{
  ...
  if (!argone || !*argone) {
    notify_player(player, cause, player, "Drop what?", 0);
    return;
  }
  if ((get_int_elt(player, LOC, &here) == -1) || !exists_object(here)) {
    notify_player(player, cause, player, noloc, 0);
  ...
  if (obj == -2) {
    notify_player(player, cause, player,
    		  "I can't tell which one you want to drop.", 0);
    return;
  }
  switch (Typeof(obj)) {
  case TYP_EXIT:
  ...
  case TYP_THING:
    if (isDARK(obj) && !controls(player, cause, here)) {
      notify_player(player, cause, player, "You can't drop that here.", 0);
      return;
    }
  case TYP_PLAYER:
  ...
  default:
    notify_player(player, cause, player, 
		  "You have no business carrying that!", 0);
    return;
  }
  notify_player(player, cause, player, "Dropped.", 0);
}
    ...
    char *argone, *argtwo;
{
  ...
  if (!argone || !*argone) {
    notify_player(player, cause, player, "Who would you like to kill?", 0);
    return;
  }
  ...
  if(mudconf.enable_money) {
    if (!argtwo || !*argtwo) {
      ...
      if(ptr == argtwo){
        notify_player(player, cause, player,
		      "You must specify cost by number.", 0);
        return;
      }
  ...
  if (isHAVEN(here) && !isWIZARD(player)) {
    notify_player(player, cause, player, "It's too peaceful here.", 0);
    return;
  }
  ...
  if (victim == -2) {
    notify_player(player, cause, player,
    		  "I can't figure out what you want to kill.", 0);
    return;
  }
  ...
  case TYP_THING:
    if (!isWIZARD(player)
    	&& (((mudconf.enable_money
	      && ((random() % mudconf.maxkill_cost) >= cost))
	    || !mudconf.enable_money) || isWIZARD(victim))) {
      /* Fail the kill */

      notify_player(player, cause, player, "Your murder attempt failed.", 0);
    ...
    break;
  default:
    notify_player(player, cause, player, "You can't kill that!", 0);
  }
}
    ...
    char *argone, *argtwo;
{
  ...
  if (!argone || !*argone) {
    notify_player(player, cause, player, "Teleport what to where?", 0);
    return;
  }
  if (!argtwo || !*argtwo) {
    object = player;
    dest = resolve_object(player, cause, argone, RSLV_NOISY | RSLV_HOME);
    if(dest == -1)
      return;
  } else {
    object = resolve_object(player, cause, argone, RSLV_NOISY);
    if(object == -1)
      return;
    dest = resolve_object(player, cause, argtwo, RSLV_NOISY | RSLV_HOME);
    if(dest == -1)
      return;
  }
  /* do it */
  if ((get_int_elt(object, LOC, &loc) == -1) || !exists_object(loc)) {
    notify_player(player, cause, player, "That object has no location!", 0);
    return;
  }
  ...
  case TYP_ROOM:		/* parent */
	...
	return;
      ...
      if (!isROOM(dest)) {
	notify_player(player, cause, player, "That's not a room!", 0);
	return;
      }
    ...
    notify_player(player, cause, player, "Location set.", 0);
    return;
  case TYP_EXIT:		/* no go */
    notify_player(player, cause, player,
    		  "You can't teleport exits.  Use @attach, instead.", 0);
    return;
  case TYP_THING:
  case TYP_PLAYER:		/* our most complex */
    ...
    if (Typeof(dest) == TYP_EXIT) {
      notify_player(player, cause, player,
		    "You can't teleport anything to that!", 0);
      return;
    }
    if (object == dest) {
      notify_player(player, cause, player,
    ...
    look_location(object, cause);

    break;
  default:
    notify_player(player, cause, player,
		  "You have no business teleporting that!", 0);
    return;
  }
  notify_player(player, cause, player, "Teleported.", 0);
}
...
static void reward_move(player, cause, loc)
    int player, cause, loc;
{
  if (mudconf.enable_money
    ...
    case 2:
    case 3:
      notify_player(player, cause, player, "You found a penny!", 0);
      reward_money(player, 1);
      break;
    case 4:
    case 5:
    case 6:
      notify_player(player, cause, player, "You found a nickel!", 0);
      reward_money(player, 5);
      break;
    case 7:
    case 8:
      notify_player(player, cause, player, "You found a dime!", 0);
      reward_money(player, 10);
      break;
    default:
      notify_player(player, cause, player, "You found a quarter!", 0);
      reward_money(player, 25);
    }
  }
}
