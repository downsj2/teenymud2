...
#ifndef MAXGROUPS
#define MAXGROUPS	128		/* Should be more than enough. */
#endif			/* MAXGROUPS */

static struct glist *garray[MAXGROUPS];
static int gtop = 0;
...
static void group_dump _ANSI_ARGS_((int, int, bool));
 ...
 */
void group_create(player, cause, str)
    int player, cause;
    char *str;
{
  if(group_ismem(player))
    notify_player(player, cause, player, "You're already in a group!", 0);
  else {
    if(gtop < MAXGROUPS) {
      struct glist *newgrp;
	...
	newgrp->str = ty_strdup(str, "group_create.str");
      ...
      notify_player(player, cause, player, "New group created.", 0);
    } else
      notify_player(player, cause, player,
		    "Sorry, there are too many groups.", 0);
  }
}
 ...
 */
void group_join(player, cause, friend)
    int player, cause, friend;
{
  struct glist *actual, *leader;

  if(group_ismem(player)) {
    notify_player(player, cause, player,
		  "You can only be in one group at a time.", 0);
    return;
  }

  if(!group_find(friend, &actual, &leader))
    notify_player(player, cause, player, "But they aren't in a group!", 0);
  else {
    struct glist *newmem;
    char *name, buf[MEDBUFFSIZ];

    if(leader->flags & GRP_CLOSE) {
      notify_player(player, cause, player, "Sorry, that group is closed.", 0);
      return;
    }
  ...
  while(curr != (struct glist *)NULL) {
    ...
    if(!(curr->flags & GRP_SILENT))
      notify_player(curr->player, cause, curr->player,
		    "**] Your group has been dissolved.", 0);
    glist_free(curr);
  ...
  if(group_find(leader, &actual, &rleader) && (actual == rleader)) {
	...
	if(loc != oldloc)	/* They aren't with the group. */
	  continue;
        ...
        look_location(curr->player, cause);
      }
    }
  }
}
 ...
 */
static void group_dump(player, cause, all)
    int player, cause;
    bool all;
{
  struct glist *actual, *leader;
  ...
  if(all) {
    ...
    if(group_find(player, &actual, &leader)) {
      ...
      notify_player(player, cause, player, "But you're not in a group!", 0);
  }
}
    ...
    char *argone;
{
  if(mudconf.enable_groups) {
    if(switches & GROUP_DISOLVE) {
      ...
      if(!group_find(player, &actual, &leader)) {
	notify_player(player, cause, player, "You're not even in a group!", 0);
	return;
      }
	...
	notify_player(player, cause, player, "You must specify a player.", 0);
	return;
      }
	...
	if((argone != (char *)NULL) &&
      ...
      group_dump(player, cause, (switches & GROUP_ALL));
    } else if(switches & GROUP_JOIN) {
      int friend;

      if((argone == (char *)NULL) || (argone[0] == '\0')) {
	notify_player(player, cause, player,
		      "You must specify a player to join.", 0);
	return;
      }
      friend = resolve_player(player, cause, argone, RSLV_NOISY);
      if(friend == -1)
	return;
      group_join(player, cause, friend);
    } else if(switches & GROUP_FOLLOW) {
      struct glist *actual, *leader;

      if(!group_find(player, &actual, &leader)) {
        notify_player(player, cause, player,
		      "You're not a member of a group!", 0);
      } else {
        if(actual == leader) {
	  notify_player(player, cause, player,
		        "You wish to follow yourself?", 0);
	  return;
        }
        actual->flags |= GRP_FOLLOW;

        notify_player(player, cause, player, "Following activated.", 0);
	...
	return;
      ...
      group_create(player, cause, argone);
    }
  } else
    notify_player(player, cause, player, "Sorry, groups are not enabled.", 0);
}
