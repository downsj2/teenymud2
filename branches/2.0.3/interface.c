    ...
    char *input;
{
  ...
  if(!strncasecmp(input, "co", 2)) {
	...
	if((mudstat.logins != LOGINS_DISABLED) || isWIZARD(player)) {
	  ...
	  if(attr_add(player, SITE, cp->host, SITE_FLGS) == -1)
	    logfile(LOG_ERROR,
	    	    "handle_login: couldn't set host on player #%d\n", player);
...
static void do_stty(cp, input)
    ...
    char *input;
{
  static const char *const stty_nlmodekey[] = {"crnl", "cr", "nl", "nlcr"};
  static const int stty_nlmodeval[] = {CONN_CRNL, CONN_CR, CONN_NL, CONN_NLCR};
  int i;
  char *ptr;
  char buf[BUFFSIZ];
    ...
    char *arg;
{
  ...
  for (cp = connlist; cp != (struct conn *) NULL; cp = cp->next) {
    ...
    if (strlen(arg) && stringprefix(name, arg)) {
    ...
    char *argone, *argtwo;
{
  ...
  if (argtwo && *argtwo) {
    person = resolve_player(player, cause, argone, RSLV_NOISY);
    if(person == -1)
      return;
  ...
  if (!isALIVE(person)) {	/* @force won't crash me */
    notify_player(player, cause, player, "That person is not connected.", 0);
    return;
  }
  ...
  notify_player(player, cause, player,
  		(person == player) ? "Set.  Have fun doing it." : "Set.", 0);
}
    ...
    char *argone;
{
  ...
  notify_player(player, cause, player, "Poll set.", 0);
}

/* this table needs to match the defines in conn.h */
static const char *const nltab[4] = { "CONN_CRNL", "CONN_CR", "CONN_NL",
				      "CONN_NLCR" };
    ...
    struct conn *cp;
{
  ...
  if((cp->player != -1) && exists_object(cp->player) && isPLAYER(cp->player)) {
	...
	if(*ptr != ':') {
	  ...
	  ptr++;
	  for(idx = 0; idx < 4; idx++) {
	    if(strncmp(nltab[idx], ptr, strlen(nltab[idx])) == 0) {
	      cp->lcrnl = idx;
	      break;
	    }
	  }
    ...
    char *argone;
{
  ...
  for (cp = connlist; cp != (struct conn *) NULL; cp = cp->next) {
    ...
    snprintf(buf, sizeof(buf), "%d:%s", cp->lwrap, nltab[cp->lcrnl]);
  ...
  notify_player(player, cause, player, "Stty settings saved.", 0);
}
