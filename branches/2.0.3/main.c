    ...
    char **argv;
{
  ...
  on_exit(report_malloc, NULL);
#endif
#if defined(MALLOC_DEBUG) && defined(HAVE_ATEXIT)
  atexit(report_malloc);
#endif
...
static RETSIGTYPE dump_finished()
{
  waitpid(dump_pid, NULL, 0);
  cache_unlock();
  mudstat.status = MUD_RUNNING;
...
static RETSIGTYPE signal_shutdown()
{
  logfile(LOG_STATUS, "Caught a shutdown signal... Closing down.\n");
  mud_shutdown(0);
}

static RETSIGTYPE signal_dump()
{
