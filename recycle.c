    ...
    char *argone;
{
  ...
  if((argone == (char *)NULL) || (argone[0] == '\0')) {
    notify_player(player, cause, player, "Recycle what?", 0);
    return;
  }
  ...
  if(!isDESTROY_OK(obj)) {
    if(!(switches & RECYCLE_OVERRIDE)) {
      ...
      if(objown != plown) {
        notify_player(player, cause, player, "You can't recycle that!", 0);
        return;
      }
  ...
  if((obj == mudconf.starting_loc) || (obj == mudconf.root_location)) {
    notify_player(player, cause, player, "You may not recycle that.", 0);
    return;
  }
  if(isPLAYER(obj)) {
    notify_player(player, cause, player, "You may not recycle players.", 0);
    return;
  }
  ...
  case -1:
    ...
    break;
  case 0:
    notify_player(player, cause, player, "Nothing to recycle?!", 0);
    break;
  case 1:
    notify_player(player, cause, player, "Thank you for recycling.", 0);
    break;
  default:
