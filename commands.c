...
static CMDS cmddefs[] =
{
 ...
 /* help */
  {"help", do_help, NULL, 1, CMDS_NORMAL},
 /* home */
 ...
 /* news */
  {"news", do_news, NULL, 1, CMDS_NORMAL},
 /* page */
  {"page", do_page, NULL, 2, CMDS_NORMAL},
 /* pose */
  {"pose", do_pose, pose_switches, 1, CMDS_NORMAL},
 /* say */
  {"say", do_say, NULL, 1, CMDS_NORMAL},
 /* score */
 ...
 /* whisper */
  {"whisper", do_whisper, NULL, 2, CMDS_NORMAL},
 /* @attach */
 ...
 /* @case */
  {"@case", do_case, case_switches, CMDS_VARARGS, CMDS_NORMAL|CMDS_NOEXEC},
 /* @children */
 ...
 /* @copy */
  {"@copy", do_copy, NULL, 2, CMDS_NGUEST},
 /* @cost */
 ...
 /* @doing */
  {"@doing", do_doing, NULL, 2, CMDS_PLAYER},
 /* @dump */
 ...
 /* @emit */
  {"@emit", do_emit, emit_switches, 1, CMDS_NORMAL},
 /* @entrances */
 ...
 /* @pemit */
  {"@pemit", do_pemit, NULL, 2, CMDS_NORMAL},
 /* @pcreate */
 ...
 /* @savestty */
  {"@savestty", do_savestty, NULL, 1, CMDS_NGUEST},
 /* @semaphore */
    ...
    char *eargv[];
{
  ...
  if(entry != NULL){
    ...
    /* parse switches. */
    if ((cptr->switches != NULL) && (swptr != NULL)) {
      /* i'd make this a routine, but i want it inline, so... */

      /* local variables. */
      register int overtotal = 0;	/* overall total. */
      register int intotal;		/* inner total. */
      register SWITCH *sptr;

      while (*swptr) {
	for (sptr = cptr->switches, intotal = 0;
	     *swptr && (sptr->name != NULL); sptr++) {
	  if (!strncasecmp(sptr->name, swptr, sptr->matlen)) {
	    ...
	    if (overtotal && (sptr->flags & SWITCH_SINGULAR)) {
	      notify_player(player, cause, player,
			  "Illegal combination of command switches.", 0);
	      if (mudstat.status != MUD_DUMPING)
		cache_unlock();
	      return;
	    } else {
	...
	if (!intotal) {
	  notify_player(player, cause, player, "Illegal command switch.", 0);
	  if (mudstat.status != MUD_DUMPING)
	    cache_unlock();
	  return;
	}
	...
	if (*swptr)
      ...
      if (!switches) {
	notify_player(player, cause, player, "Illegal switches.", 0);
	if (mudstat.status != MUD_DUMPING)
	  cache_unlock();
	return;
      }
    ...
    case 1:	/* pretty simple case -- ptr is argument one. */
      if(*ptr && (fexec || !(cptr->perms & CMDS_NOEXEC))) {
      ...
      for (i = 0; i < ATABLE_TOTAL; i++) {
        if (Atable[i].command && (stringprefix((Atable[i].calias) ?
          ...
          parse_argtwo(player, cause, ptr, &argone, &argtwo, CMDS_NOEXEC,
	  	       eargc, eargv);
          do_set_string(player, cause, argone, argtwo, &Atable[i]);
	  ty_free((VOID *)argone);
	  ty_free((VOID *)argtwo);
          if (mudstat.status != MUD_DUMPING)
            cache_unlock();
    ...
    case '&':		/* attribute. */
      ...
      parse_argtwo(player, cause, ptr, &argone, &argtwo, CMDS_NOEXEC,
      		   eargc, eargv);
      i = resolve_object(player, cause, argone, RSLV_EXITS | RSLV_NOISY);
      if (i != -1) {
        if (cmd && *cmd) {
          set_attribute(player, cause, i, cmd, argtwo);
        } else {
