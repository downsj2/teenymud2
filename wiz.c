    ...
    int player, cause, switches;
{
  ...
  notify_player(player, cause, player,
  		"Sorry, that command is not supported on this system.", 0);
#else
  struct rusage rbuf;
  char buf[MEDBUFFSIZ];
  ...
  if(getrusage(RUSAGE_SELF, &rbuf) == -1) {
    notify_player(player, cause, player, "Sorry, something is broken.", 0);
    return;
  }
  ...
  if(getloadavg(load, 3) == 3) {
    snprintf(buf, sizeof(buf), "Load: %.2f, %.2f, %.2f", load[0], load[1],
	     load[2]);
    notify_player(player, cause, player, buf, 0);
  }
    ...
    char *argone, *argtwo;
{
  ...
  if (!argone || !*argone) {
    notify_player(player, cause, player, "Boot whom?", 0);
    return;
  }
  ...
  if (isGOD(victim) && !isGOD(player)) {
    notify_player(player, cause, player, "You can't boot God!", 0);
    return;
  }
  if (argtwo && *argtwo) {
    ...
    if(ptr == argtwo) {
      notify_player(player, cause, player,
		    "You must specify an fd by number.", 0);
      return;
    }
    if (fd_conn(victim) != fd) {
      notify_player(player, cause, player,
     		    "That player isn't connected on that fd.", 0);
      return;
    }
    ...
    char *argone, *argtwo;
{
  ...
  if (!argone || !*argone || !argtwo || !*argtwo) {
    notify_player(player, cause, player, "Chown what to who?", 0);
    return;
  }
  owner = resolve_player(player, cause, argtwo, RSLV_NOISY);
  if(owner == -1)
    return;
  obj = resolve_object(player, cause, argone, RSLV_NOISY | RSLV_EXITS);
  if(obj == -1)
    return;
  if (!isWIZARD(player) && (owner != player)) {
    notify_player(player, cause, player,
		  "You may only chown things to yourself.", 0);
    return;
  }
  if (isPLAYER(obj) && !isGOD(player)) {
    notify_player(player, cause, player, "Players always own themselves.", 0);
    return;
  }
  ...
  if (mudconf.enable_money && !isWIZARD(player)
      && !can_afford(owner, cause, cost_of(obj)))
    return;
  if (mudconf.enable_quota && !isWIZARD(player) && no_quota(owner, cause))
    return;

  if ((get_int_elt(owner, OWNER, &owner) == -1) ||
      (get_int_elt(obj, OWNER, &exowner) == -1) ||
      (set_int_elt(obj, OWNER, owner) == -1)) {
    notify_player(player, cause, player, "Couldn't set owner.", 0);
  } else {
    notify_player(player, cause, player, "Owner changed.", 0);
    ...
    int player, cause, switches;
{
  if (mudstat.status == MUD_DUMPING) {
    notify_player(player, cause, player,
		  "Wait for the first one to finish, eh?", 0);
  } else {
    notify_player(player, cause, player, "Dumping...", 0);
    dump_db();
  }
}
    ...
    char *argone, *argtwo;
{
  ...
  if (!argone || !*argone) {
    notify_player(player, cause, player, "Change who's password to what?", 0);
    return;
  }
  victim = resolve_player(player, cause, argone, RSLV_NOISY);
  if(victim == -1)
    return;
  if (isGOD(victim) && !isGOD(player)) {
    notify_player(player, cause, player, "You can't change God's password!", 0);
    return;
  }
  if ((!argtwo || !*argtwo)) {
    notify_player(player, cause, player, "Change their password to what?", 0);
    return;
  }
  ...
  if (argtwo && *argtwo && (player != victim)) {
    snprintf(buf, sizeof(buf),
    ...
    char *argone, *argtwo;
{
  ...
  if (!argone || !*argone) {
    notify_player(player, cause, player, "Whom do you want to toad?", 0);
    return;
  }
  if (!argtwo || !*argtwo) {
    newowner = -1;
  } else {
    newowner = resolve_player(player, cause, argtwo, RSLV_NOISY);
    if(newowner == -1)
      return;
  ...
  if(victim == -1)
    return;
  if (isGOD(victim)) {
    notify_player(player, cause, player, "God may not be toaded.", 0);
    return;
  }
  if (player == victim) {
    notify_player(player, cause, player,
    		  "Toading yourself would be quite stupid.", 0);
    return;
  }
  if (isWIZARD(victim) && !isGOD(player)) {
    notify_player(player, cause, player, "You can't toad another wizard!", 0);
    return;
  }
  notify_player(victim, cause, player,
    ...
    int player, cause, switches;
{
  ...
  if (mudstat.status == MUD_DUMPING) {
    notify_player(player, cause, player,
		  "Wait for it to finish dumping, eh?", 0);
    return;
  } else {
...
{
  int newplayer, loc;
  char *name, *pwd, nbuf[MAXNAMELEN + 1], pbuf[9], buf[MEDBUFFSIZ];
  char *newname, *pname;

  if (!argone || !*argone || !argtwo || !*argtwo) {
    notify_player(player, cause, player,
		  "Usage: @pcreate <name> = <password>", 0);
    return;
  }
  ...
  while (*argone && *argone != '\"' && (name - nbuf) <= MAXNAMELEN) {
    if (!isspace(*argone)) {
      *name++ = *argone++;
      continue;
    }
  ...
  *name = '\0';
  ...
  if (match_player(nbuf) != -1) {
    notify_player(player, cause, player,
    		  "A player with that name already exists.", 0);
    return;
  }
  if (!ok_player_name(nbuf)) {
    notify_player(player, cause, player,
    		  "That's not a reasonable name for a player.", 0);
    return;
  }
  ...
  *pwd = '\0';
  ...
  if(newplayer == -1)
    notify_player(player, cause, player, "Pcreate failed.", 0);
  else {
    int owner;
    ...
    char *argone;
{
  ...
  if (!argone || !*argone) {
    notify_player(player, cause, player,
    		  "But whose possessions do you want to purge?", 0);
    return;
  }
  victim = resolve_player(player, cause, argone, RSLV_NOISY);
  if(victim == -1)
  ...
  if (isGOD(victim)) {
    notify_player(player, cause, player,
		  "You may not purge God's possessions.", 0);
    return;
  }
  snprintf(buf, sizeof(buf),
	   "Purged %d objects.", recycle_owned(player, cause, victim));
  notify_player(player, cause, player, buf, 0);
}
    ...
    char *argone, *argtwo;
{
  ...
  if (!argone || !*argone || !argtwo || !*argtwo) {
    notify_player(player, cause, player,
    		  "Usage: @chownall <oldowner> = <newowner>", 0);
    return;
  }
  oldowner = resolve_player(player, cause, argone, RSLV_NOISY);
  if(oldowner == -1)
    return;
  newowner = resolve_player(player, cause, argtwo, RSLV_NOISY);
  if(newowner == -1)
  ...
  if (isGOD(oldowner)) {
    notify_player(player, cause, player,
    		  "You may not chown all of God's possessions.", 0);
    return;
  }
  snprintf(buf, sizeof(buf), "Chowned %d objects to #%d.",
	   chownall(oldowner, newowner), newowner);
  notify_player(player, cause, player, buf, 0);
}
  ...
  if(mudconf.enable_quota) {
	...
	return;
      ...
      if (!controls(player, cause, victim) || !has_quota(victim)) {
        notify_player(player, cause, player,
		      "You're simply not perceptive enough.", 0);
        return;
      }
    }
    if (!argtwo || !*argtwo) {
      ...
      if (!isWIZARD(player)) {
        notify_player(player, cause, player,
      		      "Contact a Wizard if your quota needs updated.", 0);
        return;
      }
      quota = (int)strtol(argtwo, &ptr, 10);
      if((ptr == argtwo) || (quota < 0)) {
        notify_player(player, cause, player,
      		      "Please specify only a positive number.", 0);
        return;
      }
      if (set_int_elt(victim, QUOTA, quota) == -1) {
        notify_player(player, cause, player, "Couldn't set quota.", 0);
        return;
      }
    ...
    notify_player(player, cause, player, "Quotas are not enabled.", 0);
}
    ...
    char *argone;
{
  ...
  if (!argone || !*argone) {
    notify_player(player, cause, player, "You must specify an amount.", 0);
    return;
  }
  amount = (int)strtol(argone, &ptr, 10);
  if(ptr == argone) {
    notify_player(player, cause, player,
		  "You must specify a numerical amount.", 0);
    return;
  }
    ...
    char *argone, *argtwo;
{
  ...
  if(argone != (char *)NULL) {
    ...
    default:
      notify_player(player, cause, player,
		    "I don't understand that argument.", 0);
      return;
    }
  } else
    wh = MOTD_ANY;
  ...
  if(switches & MOTD_SET) {
    ...
    case MOTD_NORM:
      ...
      break;
    default:
      notify_player(player, cause, player, "You can't set just any motd!", 0);
      return;
    }
    notify_player(player, cause, player, "Motd set.", 0);
  } else {
