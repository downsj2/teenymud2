    ...
    int player, cause, switches;
{
  ...
  snprintf(buf, sizeof(buf),
	   "Max. RSS: %ld, minor faults: %ld, major faults: %ld, swaps: %ld",
  	   rbuf.ru_maxrss, rbuf.ru_minflt, rbuf.ru_majflt, rbuf.ru_nswap);
  notify_player(player, cause, player, buf, 0);
  snprintf(buf, sizeof(buf),
	   "Input block I/O: %ld, output block I/O: %ld", rbuf.ru_inblock,
  	   rbuf.ru_oublock);
  notify_player(player, cause, player, buf, 0);
  snprintf(buf, sizeof(buf),
	   "Messages sent: %ld, messages recvd: %ld", rbuf.ru_msgsnd,
  	   rbuf.ru_msgrcv);
  notify_player(player, cause, player, buf, 0);
  snprintf(buf, sizeof(buf),
    "Signals: %ld, vol. context switches: %ld, forced context switches: %ld",
