...
static struct boolexp *parse_boolexp_D()
{
  ...
  *ptr-- = '\0';
  ...
  if(strncasecmp(buf, "flag:", 5) == 0) {
    ...
    b->type = BOOLEXP_FLAG;
    /* GenFlags */
    idx = match_flaglist_name(buf + 5, GenFlags, GenFlags_count);
    if (idx >= 0) {
      ((b->dat).flags)[GenFlags[idx].word] = GenFlags[idx].code;
      return(b);
    }
    /* PlayerFlags */
    idx = match_flaglist_name(buf + 5, PlayerFlags, PlayerFlags_count);
    if (idx >= 0) {
      ((b->dat).flags)[PlayerFlags[idx].word] = PlayerFlags[idx].code;
      return(b);
    }
    /* ThingFlags */
    idx = match_flaglist_name(buf + 5, ThingFlags, ThingFlags_count);
    if (idx >= 0) {
      ((b->dat).flags)[ThingFlags[idx].word] = ThingFlags[idx].code;
      return(b);
    }
    /* RoomFlags */
    idx = match_flaglist_name(buf + 5, RoomFlags, RoomFlags_count);
    if (idx >= 0) {
      ((b->dat).flags)[RoomFlags[idx].word] = RoomFlags[idx].code;
      return(b);
    }
    /* ExitFlags */
    idx = match_flaglist_name(buf + 5, ExitFlags, ExitFlags_count);
    if (idx >= 0) {
      ((b->dat).flags)[ExitFlags[idx].word] = ExitFlags[idx].code;
      return(b);
    }
    ...
    int outer_type;
{
  if (b == (struct boolexp *) NULL) {
	...
	*buftop++ = ')';
	...
	if(GenFlags[idx].code == ((b->dat).flags)[GenFlags[idx].word]) {
	  strcpy(buftop, "flag:");
	  strcat(buftop, GenFlags[idx].name);
	  matched++;
	}
      }
      if(!matched) {
	...
	for(idx = 0; idx < PlayerFlags_count; idx++) {
	  if(PlayerFlags[idx].code == ((b->dat).flags)[PlayerFlags[idx].word]) {
	    strcpy(buftop, "flag:");
	    strcat(buftop, PlayerFlags[idx].name);
	    matched++;
	  }
	}
      }
	...
	for(idx = 0; idx < RoomFlags_count; idx++) {
	  if(RoomFlags[idx].code == ((b->dat).flags)[RoomFlags[idx].word]) {
	    strcpy(buftop, "flag:");
	    strcat(buftop, RoomFlags[idx].name);
	    matched++;
	  }
	}
      }
	...
	for(idx = 0; idx < ThingFlags_count; idx++) {
	  if(ThingFlags[idx].code == ((b->dat).flags)[ThingFlags[idx].word]) {
	    strcpy(buftop, "flag:");
	    strcat(buftop, ThingFlags[idx].name);
	    matched++;
	  }
	}
      }
	...
	for(idx = 0; idx < ExitFlags_count; idx++) {
	  if(ExitFlags[idx].code == ((b->dat).flags)[ExitFlags[idx].word]) {
	    strcpy(buftop, "flag:");
	    strcat(buftop, ExitFlags[idx].name);
	    matched++;
	  }
	}
      }
