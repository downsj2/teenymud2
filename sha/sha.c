...
static void byteReverse _ANSI_ARGS_((LONG *, int));
...
void shsUpdate(shsInfo, buffer, count)
    ...
    bcopy((char *)buffer, (char *)shsInfo->data, count);
    }
...
void shsFinal(shsInfo)
	...
	bzero((char *)shsInfo->data + count, 64 - count);
#ifndef WORDS_BIGENDIAN
	byteReverse( shsInfo->data, SHS_BLOCKSIZE );
#endif			/* WORDS_BIGENDIAN */
	shsTransform( shsInfo );
	...
	bzero((char *)shsInfo->data, 56);
	}
    else
	/* Pad block to 56 bytes */
	bzero((char *)shsInfo->data + count, 56 - count);
#ifndef WORDS_BIGENDIAN
    byteReverse( shsInfo->data, SHS_BLOCKSIZE );
#endif			/* WORDS_BIGENDIAN */
