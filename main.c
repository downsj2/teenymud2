...
#if defined(HAVE_SETRLIMIT) && defined(RLIMIT_NOFILE) && !defined(__APPLE__)
    ...
    char **argv;
{
  ...
  fputs("TeenyMUD is free software and comes with absolutely no warranty.\n",
  	stdout);
  fputs("Please see the file ``COPYING'' for more information.\n", stdout);
...
static void rlimit_init()
{
  ...
  if (getrlimit(RLIMIT_NOFILE, &rlp) == 0) {
    ...
    if (setrlimit(RLIMIT_NOFILE, &rlp) != 0)
      logfile(LOG_ERROR, "rlimit_init: setrlimit %d failed, %s\n",
	      rlp.rlim_cur, strerror(errno));
  } else
