...
int parse_name_pwd(str, name, pwd)
    char *str, **name, **pwd;
{
  static char nbuf[MAXNAMELEN + 1], pbuf[9];
  char *p;
  ...
  if (*str != '\"') {		/* the name isn't quoted, fuck */
    ...
    for (p = nbuf; *str && !isspace(*str) && (p - nbuf) <= MAXNAMELEN;
	 *p++ = *str++);
    if (*str == '\0')
      return (-1);
    *p = '\0';
    ...
    *name = nbuf;
    ...
    p = nbuf;
    *name = nbuf;
    while (*str && *str != '\"' && (p - nbuf) <= MAXNAMELEN) {
      if (!isspace(*str)) {
	*p++ = *str++;
	continue;
      }
    ...
    char *name;
{
  ...
  for (i = 0; name[i]; i++) {
	...
	(name[i] == '[') || (name[i] == ']') || !isprint(name[i]))
      return (0);
  }
  return (strcmp(name, "A") && strcmp(name, "An") && strcmp(name, "The")
	  && strcmp(name, "You") && strcmp(name, "Your")
   && strcmp(name, "Going") && strcmp(name, "Huh?") && strcmp(name, "me")
	  && strcmp(name, "home") && strcmp(name, "here"));
}

int ok_player_name(name)
    char *name;
{
  if (!ok_name(name) || (strlen(name) > MAXNAMELEN) ||
      (match_player(name) != -1))
    return (0);
  else
    return (1);
...
int can_afford(player, cause, amount)
    int player, cause, amount;
{
  int pennies, owner;
  ...
  notify_player(player, cause, player, "You don't have enough pennies.", 0);
  return (0);
}
